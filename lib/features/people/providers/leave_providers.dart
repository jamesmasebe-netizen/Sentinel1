...
final leaveRequestsCollectionProvider =
    Provider<CollectionReference<LeaveRequest>>((ref) {
      ...
      return firestore
          ...
          .withConverter<LeaveRequest>(
            fromFirestore:
                (snapshot, _) =>
                    LeaveRequest.fromMap(snapshot.data()!, snapshot.id),
...
class LeaveService {
  ...
  Future<void> updateLeaveRequest(LeaveRequest request) async {
    await _firestore
        .tenantCollection(_tenantId, 'leaveRequests')
        .doc(request.id)
