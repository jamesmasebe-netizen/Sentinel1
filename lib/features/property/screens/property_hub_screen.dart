...
class _PropertyHubScreenState extends ConsumerState<PropertyHubScreen> {
  ...
  Widget _buildStatsRow(List<Property> properties) {
    int criticalCount =
        properties.where((p) => p.status.contains('Critical')).length;
    double avgOccupancy =
        properties.isEmpty
            ? 0
            : (properties.fold(0, (sum, p) => sum + p.occupancy) /
                    properties.fold(0, (sum, p) => sum + p.capacity)) *
                100;
    double avgCompliance =
        properties.isEmpty
            ? 0
            : properties.fold(0.0, (sum, p) => sum + p.complianceScore) /
                properties.length;
