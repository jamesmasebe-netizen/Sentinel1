...
class DashboardPrince2Overview extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final completedStages = projects.fold<int>(
      0,
      (s, p) => s + (p.stages).where((st) => st.status == 'Completed').length,
    );
    final totalStages = projects.fold<int>(0, (s, p) => s + (p.stages).length);
    final stageProgress =
        totalStages == 0 ? 0.0 : completedStages / totalStages;
    final criticalTaskCount = projects.fold<int>(
      0,
      (s, p) => s + (p.tasks).where((t) => t.riskLevel == 'Critical').length,
    );
    final overallPortfolioProgress =
        projects.isEmpty
            ? 0.0
            : projects.fold(0.0, (s, p) => s + (p.overallProgress)) /
                projects.length;
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const Spacer(),
              Text(
                '${(overallPortfolioProgress * 100).toStringAsFixed(1)}%',
                style: const TextStyle(
                  fontSize: 12,
            ...
            builder: (context, constraints) {
              ...
              final domains = [
                DashboardPrince2DomainTile(
                  ...
                  label: 'Finance',
                  color: XMTheme.info,
                  headline: NumberFormat.compactCurrency(
                    symbol: 'R',
                  ).format(totalBudget),
                  detail: 'Total portfolio budget',
                  status: totalBudget > 5000000 ? 'High Spend' : 'Normal',
                  statusColor:
                      totalBudget > 5000000 ? XMTheme.warning : XMTheme.success,
                ),
                DashboardPrince2DomainTile(
                ...
                DashboardPrince2DomainTile(
                  ...
                  status:
                      totalNcrs == 0 ? 'No Open NCRs' : '$totalNcrs Open NCRs',
                ...
                DashboardPrince2DomainTile(
                  ...
                  statusColor:
                      stageProgress >= 0.7 ? XMTheme.success : XMTheme.warning,
                ),
                DashboardPrince2DomainTile(
                  ...
                  detail:
                      '$draftCount Draft • $onHoldCount On Hold • $completedCount Done',
                  status: onHoldCount > 0 ? '$onHoldCount On Hold' : 'Healthy',
                  statusColor:
                      onHoldCount > 0 ? XMTheme.warning : XMTheme.success,
                ),
                DashboardPrince2DomainTile(
                  ...
                  headline:
                      '${(overallPortfolioProgress * 100).toStringAsFixed(0)}%',
                  ...
                  status:
                      criticalTaskCount > 0
                          ...
                          : 'On Track',
                  statusColor:
                      criticalTaskCount > 0 ? XMTheme.error : XMTheme.success,
                ),
              ];
              if (isWide) {
              ...
              return Column(
                children:
                    domains
                        .expand((d) => [d, const SizedBox(height: 10)])
                        .toList()
                      ..removeLast(),
              );
            },
