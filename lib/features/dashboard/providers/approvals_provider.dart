...
final pendingApprovalsProvider = StreamProvider<List<ApprovalItem>>((ref) async* {
...
final pendingApprovalsFutureProvider = FutureProvider.autoDispose<List<ApprovalItem>>((ref) async {
  final userProfile = ref.watch(userProfileProvider).valueOrNull;
  ...
  // 1. Leave Requests
  final leaves = await FirebaseFirestore.instance.tenantCollection(ref.watch(currentTenantIdProvider) ?? "", 'leave_requests')
  ...
  final reqs = await ref.watch(firestoreProvider).tenantCollection(ref.watch(currentTenantIdProvider) ?? "", 'job_requisitions')
      .where('siteId', isEqualTo: siteId)
      .where('hiringManagerId', isEqualTo: userProfile.uid)
      .where('status', isEqualTo: 'Draft') // Using Draft as pending approval
      .get();
