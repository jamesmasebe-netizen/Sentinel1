...
final dashboardLtifrHistoryProvider = StreamProvider<List<double>>((ref) {
  ...
  return firestore.tenantCollection(ref.watch(currentTenantIdProvider) ?? "", 'incidents')
...
final dashboardOhsComplianceProvider = StreamProvider<double>((ref) {
  ...
  return firestore.tenantCollection(ref.watch(currentTenantIdProvider) ?? "", 'competency_passports')
      .where('siteId', isEqualTo: siteId)
      .snapshots()
      .map((s) {
        if (s.docs.isEmpty) return 94.0;
        final valid = s.docs.where((d) => d.data()['status'] == 'Valid' || d.data()['status'] == 'Expiring Soon').length;
...
final dashboardHiraMatrixProvider = StreamProvider<List<int>>((ref) {
  ...
  return firestore.tenantCollection(ref.watch(currentTenantIdProvider) ?? "", 'risk_assessments')
      .where('siteId', isEqualTo: siteId)
      .snapshots()
      .map((s) {
        if (s.docs.isEmpty) return defaultCounts;
        final matrix = List.filled(25, 0);
        final likelihoodMap = {'rare': 0, 'unlikely': 1, 'possible': 2, 'likely': 3, 'almost certain': 4, 'almost_certain': 4};
        final severityMap = {'catastrophic': 0, 'major': 1, 'moderate': 2, 'minor': 3, 'negligible': 4};
        
        for (final doc in s.docs) {
          final data = doc.data();
          final lStr = (data['likelihood'] ?? '').toString().toLowerCase().trim();
...
final dashboardTrainingProvider = StreamProvider<Map<String, double>>((ref) {
  final siteId = ref.watch(currentTenantIdProvider);
  final firestore = ref.watch(firestoreProvider);
  if (siteId == null) return Stream.value({'First Aid': 90.0, 'Fire Fighting': 65.0, 'SHE Rep': 100.0});
  return firestore.tenantCollection(ref.watch(currentTenantIdProvider) ?? "", 'competency_passports')
      .where('siteId', isEqualTo: siteId)
      .snapshots()
      .map((s) {
        ...
        for (final doc in s.docs) {
          ...
          if (cert.contains('first aid')) {
            category = 'First Aid';
          } else if (cert.contains('rigging') || cert.contains('fire') || cert.contains('forklift')) {
        ...
        if (total['First Aid'] == 0 && total['Fire Fighting'] == 0 && total['SHE Rep'] == 0) {
          return {'First Aid': 90.0, 'Fire Fighting': 65.0, 'SHE Rep': 100.0};
        }
        return {
          'First Aid': total['First Aid'] == 0 ? 0.0 : (valid['First Aid']! / total['First Aid']!) * 100,
          'Fire Fighting': total['Fire Fighting'] == 0 ? 0.0 : (valid['Fire Fighting']! / total['Fire Fighting']!) * 100,
          'SHE Rep': total['SHE Rep'] == 0 ? 0.0 : (valid['SHE Rep']! / total['SHE Rep']!) * 100,
        };
      });
});
...
final dashboardCapaProvider = StreamProvider<Map<String, double>>((ref) {
  ...
  return firestore.tenantCollection(ref.watch(currentTenantIdProvider) ?? "", 'capas')
      .where('siteId', isEqualTo: siteId)
      .snapshots()
      .map((s) {
...
final dashboardWasteProvider = StreamProvider<Map<String, double>>((ref) {
  final siteId = ref.watch(currentTenantIdProvider);
  final firestore = ref.watch(firestoreProvider);
  if (siteId == null) return Stream.value({'Recycle': 1.5, 'General': 2.0, 'Haz': 0.5});
  return firestore.tenantCollection(ref.watch(currentTenantIdProvider) ?? "", 'waste_manifests')
      .where('siteId', isEqualTo: siteId)
      .snapshots()
      .map((s) {
...
final dashboardIncidentHeatmapProvider = StreamProvider<List<Map<String, double>>>((ref) {
  final siteId = ref.watch(currentTenantIdProvider);
  final firestore = ref.watch(firestoreProvider);
      ...
      .map((s) {
        ...
        for (final doc in s.docs) {
          ...
          points.add({
            'x': xPct,
            'y': yPct,
            'isCritical': severity == 'Critical' || severity == 'Major' ? 1.0 : 0.0,
          });
        }
        return points;
      });
