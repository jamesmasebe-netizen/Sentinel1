      ...
      child: Column(
        ...
        children: [
          ...
          StreamBuilder<QuerySnapshot>(
            stream: firestore.tenantCollection(ref.watch(currentTenantIdProvider) ?? "", 'dynamic_risk_assessments').where('siteId', isEqualTo: siteId).snapshots(),
              ...
              return LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  return GridView.count(
                    ...
                    children: [
                      KpiCard(label: 'Total Risks', value: '${docs.length}', icon: Icons.shield_outlined, color: theme.colorScheme.primary),
                      KpiCard(label: 'Extreme', value: '$extreme', icon: Icons.warning_amber_rounded, color: XMTheme.riskExtreme),
                      KpiCard(label: 'High', value: '$high', icon: Icons.priority_high_rounded, color: XMTheme.riskHigh),
                      KpiCard(label: 'Medium', value: '$medium', icon: Icons.remove_circle_outline_rounded, color: XMTheme.riskMedium),
                      KpiCard(label: 'Low', value: '$low', icon: Icons.check_circle_outline_rounded, color: XMTheme.riskLow),
                      KpiCard(label: 'Pending', value: '$unapproved', icon: Icons.hourglass_empty_rounded, color: XMTheme.warning),
                    ],
                  );
                },
              );
  ...
  void _showDetails(BuildContext context, Map<String, dynamic> data) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('Assessor: ${data['assessorName'] ?? 'Unknown'}\nRisk Level: ${data['riskLevel'] ?? 'Low'}'),
        actions: [
