...
class BillingService {
  ...
  Future<void> createStripeCheckoutSession(
    String tenantId, {
    String? priceId,
  }) async {
    try {
  ...
  /// Opens Stripe's hosted Billing Portal, where a customer can downgrade,
  /// cancel, or update payment details on their existing subscription.
  Future<void> openBillingPortal(String tenantId) async {
    try {
      ...
      throw Exception('Failed to open billing portal: $e');
    }
  }
    ...
    return _firestore
        ...
        .map((snapshot) {
          if (snapshot.exists && snapshot.data() != null) {
            return TenantSubscription.fromFirestore(snapshot);
          }
          return null;
        });
  }
