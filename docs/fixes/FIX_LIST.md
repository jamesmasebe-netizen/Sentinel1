...
**Current behavior:** `project_lifecycle_bpf.dart` (Project Concept to Close) has 4 stage definitions but **zero orchestrator methods reference its stage IDs** (`concept`/`planning`/`execution`/`closure`) anywhere — `BpfRibbonWidget` renders on `ProjectDetailsScreen` but nothing advances it automatically (note: `projects` separately has its own real, working PRINCE2-style stage-gating system via `Project.stages`/`ProjectService.approveStage()` — this fix is about the *shared BPF engine's* representation of the same journey, which is currently decorative; decide whether to (a) wire the BPF stage to mirror `ProjectService.approveStage()`'s transitions, or (b) remove the ribbon from this screen if the module's own stage system is the intended source of truth — this is a product decision, not purely mechanical). Similarly, Procure-to-Pay's orchestrator only has `createInvoiceFromPurchaseOrder()` (the final PO→AP-Invoice step) — earlier stages (PO creation, SHEQ compliance verification, goods receipt) have no orchestrator hook at all.

**Required fix:** For Project-Concept-to-Close: after the product decision above is made, either add orchestrator methods that call `advanceStage()` at the same points `ProjectService.approveStage()` already fires, or remove `BpfRibbonWidget` from `ProjectDetailsScreen` and note in `docs/modules/projects.md` that this BPF is superseded by the module's own stage system. For Procure-to-Pay: add orchestrator methods for PO creation (`startBpf` + initial stage) and goods receipt (`advanceStage` to reflect inventory update), calling through to the real `ScmService` writes that already exist for these actions in the UI, the same pattern as F-005/F-006/F-007.
...
**Required fix:** Per `equipment.md` §8's own open question, decide whether `'Locked Out'` should be (a) added to the create/edit form's dropdown as a manually-settable option, or (b) kept exclusively automation-driven, with the real fix being a UI trigger for `LotoAutomation.lockoutFailedEquipment()` (e.g. a "Report Failure" action on `AssetDetailScreen`, naturally paired with fixing that screen's no-op edit button — see the companion item for that bug) rather than adding it to a freely-editable dropdown. Lockout exists specifically to prevent equipment being casually returned to "Operational" by mistake, which argues for (b) as the safer default — but confirm this with whoever owns the safety-workflow decision before implementing rather than guessing, per AGENTS.md §7.

**Verification:** If (a): create/edit an equipment item, select "Locked Out," confirm `LotoBadge`, `AssetDetailScreen`'s conditional badge, and `LotoManagementScreen` all correctly reflect it. If (b): trigger `lockoutFailedEquipment()` from its new UI entry point, confirm the same three read surfaces reflect it, and confirm the manual create/edit dropdown still correctly excludes "Locked Out" as a directly-selectable option.
...
**Severity:** High
**Module(s) / File(s):** `lib/features/safety/screens/contractor_qr_passport_screen.dart`, `employee_qr_passport_screen.dart`; `lib/features/auth/screens/enterprise_sso_screen.dart`; `lib/features/operations/screens/inventory_dashboard_screen.dart`; `lib/features/supply_chain/` (3 detail screens + 3 forms — enumerate via `grep -rL` for files never referenced by a route or `showSideSheet` call); `lib/features/ai_tools/screens/copilot_screen.dart` + `copilot_chat_widget.dart` + `rag_service.dart`
**Depends on:** F-319 for the `copilot_screen.dart` case specifically (its "Upgrade Now" button calls a nonexistent Cloud Function — separate bug, fixed there; this item only adds the missing entry point, don't fix the button here too)
**Source:** `docs/modules/_known_gaps_rollup.md` §1.7; `safety.md`, `auth.md`, `operations.md`, `supply_chain.md`, `ai_tools.md` §7
...
### F-018: Decide and implement copilot/ai_tools AI-chat consolidation
**Severity:** Medium
**Module(s) / File(s):** `lib/features/copilot/`, `lib/features/ai_tools/`
**Depends on:** none (but F-014's `copilot_screen.dart` entry-point fix should wait on this decision, see that item's `Depends on`)
...
**Required fix:** Product decision on which of the 3 is the intended long-term implementation (or whether they serve genuinely different purposes worth keeping separate — e.g. one as a floating contextual assistant, one as a dedicated full-screen chat). Once decided: consolidate or clearly differentiate, remove the losing implementation(s) entirely rather than leaving them as dead code, and only then apply F-014's entry-point fix to whichever survives.

**Verification:** N/A until the product decision is made — same pattern as F-017.
