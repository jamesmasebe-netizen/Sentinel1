...
## 5. Antigravity Agent Workflows & Tool Usage
- **Artifacts for Planning:** When designing a complex new feature or module, you MUST first generate a Markdown Artifact (e.g., `module_design.md`) outlining the data layer, UI components, and Riverpod structure. Set `RequestFeedback: true` to get user approval before writing code.
- **Resolving Ambiguity:** NEVER guess undefined requirements. If a requirement is ambiguous or requires a product decision, use the `ask_question` tool to present multiple-choice options to the user, or simply ask for clarification in chat.
- **Subagent Swarms (`invoke_subagent`):** For large scaffolding tasks, the parent agent should act as an orchestrator. Define specialized subagents (e.g., `UILayerAgent`, `DataLayerAgent`) using `define_subagent` or by invoking existing swarm skills (like `sentinel-swarm-orchestrator`). Do not attempt to build a massive 20-file module in a single shot.
